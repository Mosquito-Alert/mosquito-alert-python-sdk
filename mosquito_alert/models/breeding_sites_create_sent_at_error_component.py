...
class BreedingSitesCreateSentAtErrorComponent(BaseModel):
    """
    ...
    def attr_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['sent_at']):
            raise ValueError("must be one of enum values ('sent_at')")
        return value

    @field_validator('code')
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['date', 'invalid', 'make_aware', 'null', 'overflow', 'required']):
            raise ValueError("must be one of enum values ('date', 'invalid', 'make_aware', 'null', 'overflow', 'required')")
        return value
