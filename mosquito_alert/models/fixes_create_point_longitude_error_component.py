...
class FixesCreatePointLongitudeErrorComponent(BaseModel):
    """
    ...
    def attr_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['point.longitude']):
            raise ValueError("must be one of enum values ('point.longitude')")
        return value

    @field_validator('code')
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['invalid', 'max_string_length', 'null', 'required']):
            raise ValueError("must be one of enum values ('invalid', 'max_string_length', 'null', 'required')")
        return value
