...
class BreedingSitesCreatePhotosNonFieldErrorsErrorComponent(BaseModel):
    """
    ...
    def attr_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['photos.non_field_errors']):
            raise ValueError("must be one of enum values ('photos.non_field_errors')")
        return value

    @field_validator('code')
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['not_a_list', 'null', 'required']):
            raise ValueError("must be one of enum values ('not_a_list', 'null', 'required')")
        return value
