...
class PhotosPredictionCreatePredictedClassErrorComponent(BaseModel):
    """
    ...
    def attr_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['predicted_class']):
            raise ValueError("must be one of enum values ('predicted_class')")
        return value

    @field_validator('code')
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['invalid_choice', 'null']):
            raise ValueError("must be one of enum values ('invalid_choice', 'null')")
        return value
