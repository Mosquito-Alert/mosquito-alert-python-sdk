...
class IdentificationtasksPredictionsPartialUpdateScoresNonFieldErrorsErrorComponent(BaseModel):
    """
    ...
    def attr_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['scores.non_field_errors']):
            raise ValueError("must be one of enum values ('scores.non_field_errors')")
        return value
